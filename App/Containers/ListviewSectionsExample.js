...
class ListviewSectionsExample extends React.Component {
  constructor (props) {
    ...
    /* ***********************************************************
    * STEP 1
    * This is an array of objects with the properties we want
    * This will eventually come from Redux mapStateToProps
    *************************************************************/
    const dataObjects = {
      first: [
  ...
  * It's Makeme's best practice to place a single component here:
  *
  * e.g.
    return <MyCustomCell title={rowData.title} description={rowData.description} />
  *************************************************************/
  renderRow (rowData, sectionID) {
    // We can condition on sectionID (key as string), for different cells
  ...
  /* ***********************************************************
  * STEP 4
  * Since oour datasource is driven by Redux, we'll need to
  * reset it when new data arrives.
  * Please DO NOT place `cloneWithRowsAndSections` inside of render, since render
  ...
  render () {
    return (
      <View style={styles.container}>
        <AlertMessage title='No data has been found. Are you sure we did this right?' show={this.noRowData()} />
        <ListView
