...
if (Config.useReactotron) {
  Reactotron
    .configure({
      ...
      name: 'Makeme' // would you like to see the app's name?
    })
    ...
    .use(reactotronRedux({
      // we can flag some of our actions as important by returning true here
      isActionImportant: action => action.type === StartupTypes.STARTUP,

      // we can flag to exclude certain types too... especially the chatty ones
  ...
  // Totally hacky, but this allows us to not both importing reactotron-react-native
  // on every file.  This is just DEV mode, so no big deal.
  console.tron = Reactotron
} else {
  // a mock version should we decide to leave console.tron in our codebase
  console.tron = {
    log: () => false,
    warn: () => false,
    error: () => false,
